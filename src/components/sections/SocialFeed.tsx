...
function SkeletonCard() {
  return (
    <div className="absolute inset-0 rounded-xl bg-gray-50 z-10 overflow-hidden">
...
export default function SocialFeed() {
  ...
  useEffect(() => {
    // Watch for Facebook's iframe insertion into the container
    const fbObserver = new MutationObserver(() => {
      if (fbContainerRef.current?.querySelector("iframe")) {
        setFbLoaded(true);
        fbObserver.disconnect();
      }
    });
    if (fbContainerRef.current) {
      fbObserver.observe(fbContainerRef.current, { childList: true, subtree: true });
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          ...
          <FadeUp delay={0.1} className="flex flex-col items-center">
            ...
            <div
              ...
              style={{ minHeight: 600 }}
            >
              {!fbLoaded && <SkeletonCard />}
              <div
                className={`fb-page transition-opacity duration-500 ${fbLoaded ? "opacity-100" : "opacity-0"}`}
          ...
          <FadeUp delay={0.2} className="flex flex-col items-center">
            ...
            <div
              ...
              style={{ minHeight: 600 }}
            >
              {!twitterLoaded && <SkeletonCard />}
              <div
                ref={twitterRef}
                className={`w-full transition-opacity duration-500 ${twitterLoaded ? "opacity-100" : "opacity-0"}`}
                style={{ minHeight: 600 }}
              />
            </div>
          </FadeUp>

          {/* Instagram — placeholder until Behold.so widget is set up */}
